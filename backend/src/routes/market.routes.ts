...
import { screenerQuerySchema, stockListQuerySchema } from '../validators/market.validator';

const router = Router();

router.get('/overview', MarketController.getOverview);
router.get('/health', MarketController.getHealth);
router.get('/stocks', validate({ query: stockListQuerySchema }), MarketController.getStocks);
