...
export class FinanceController {
  ...
  static async createGoal(req: Request, res: Response, next: NextFunction) { try { sendSuccess(res, await FinanceService.createGoal(req.user!.id, req.body), 201, 'Live'); } catch (error) { next(error); } }
  static async updateGoal(req: Request, res: Response, next: NextFunction) { try { sendSuccess(res, await FinanceService.updateGoal(req.params.id, req.user!.id, req.body)); } catch (error) { next(error); } }
