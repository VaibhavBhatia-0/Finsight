...
export class BacktestController {
  static async run(req: Request,res: Response,next: NextFunction) { try { const test=process.env.NODE_ENV==='test'; sendSuccess(res, await BacktestService.run(req.user!.id, req.body), 201, test ? 'SYNTHETIC' : 'Historical', { source: test ? 'FINSIGHT_TEST_FIXTURE' : 'YAHOO_FINANCE_CHART', degraded: false }); } catch(error) { next(error); } }
