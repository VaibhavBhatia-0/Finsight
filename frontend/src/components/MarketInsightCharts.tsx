...
export function IndexPerformanceChart({ indices }: { indices: MarketIndex[] }) {
  return <DivergingBars title="Index session change" rows={indices.flatMap(index => index.changePercent == null ? [] : [{ label: index.name, value: index.changePercent }])} />;
}

export function MarketMoversChart({ stocks }: { stocks: MarketStock[] }) {
  const movers = stocks.filter(stock => stock.quote?.changePercent != null).slice().sort((a, b) => Math.abs(b.quote!.changePercent!) - Math.abs(a.quote!.changePercent!)).slice(0, 6);
