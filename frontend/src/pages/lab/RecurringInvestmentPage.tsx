...
interface FormValues { asset: string; amount: number; frequency: 'MONTHLY' | 'QUARTERLY' | 'ANNUALLY'; startDate: string; endDate: string; currency: string; benchmark?: string; taxJurisdiction?: 'IN' | 'US' }

export default function RecurringInvestmentPage() {
  const { register, handleSubmit, reset, control } = useForm<FormValues>({ defaultValues: { asset: '', currency: 'INR', frequency: 'MONTHLY' } });
  ...
  const submit: SubmitHandler<FormValues> = async data => {
    const request: ScenarioRequest = { scenarioType: 'RECURRING_INVESTMENT', symbol: data.asset, initialAmount: data.amount, contributionFrequency: data.frequency, startDate: data.startDate, endDate: data.endDate, baseCurrency: data.currency, benchmarkCode: data.benchmark, taxJurisdiction: data.taxJurisdiction || undefined };
  ...
  return <main className="scenario-page">
    ...
    <form onSubmit={handleSubmit(submit)} className="panel scenario-form space-y-4"><div className="panel-header"><div><h2 className="panel-title">Contribution schedule</h2><p className="panel-subtitle">Each occurrence is retained for XIRR</p></div><span className="freshness-badge">Synthetic history</span></div>
      ...
      <div className="grid grid-cols-2 gap-4"><label className="block text-sm font-medium">Contribution amount<input type="number" min="0.01" step="0.01" {...register('amount', { required: true, min: 0.01, valueAsNumber: true })} className="mt-1 w-full px-3 py-2" /></label><label className="block text-sm font-medium">Frequency<select {...register('frequency')} className="mt-1 w-full px-3 py-2"><option value="MONTHLY">Monthly</option><option value="QUARTERLY">Quarterly</option><option value="ANNUALLY">Annually</option></select></label></div>
