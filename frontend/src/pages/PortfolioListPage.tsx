...
export default function PortfolioListPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    await create.mutateAsync({ name: String(data.get('name')), baseCurrency: String(data.get('baseCurrency')) });
    form.reset(); setShowForm(false);
  }

  return <main>
    <header className="page-heading"><div><p className="page-eyebrow">Invest</p><h1>Portfolios</h1><p className="page-subtitle">Ledger-backed portfolios with chronological transactions, historical FX, and reconciled holdings.</p></div><button onClick={() => setShowForm(value => !value)} className="btn-primary inline-flex items-center gap-2 px-4 py-2"><Plus size={16} /> New portfolio</button></header>
    {showForm && <form onSubmit={submit} className="panel mb-5 flex flex-wrap gap-3"><input name="name" required maxLength={100} placeholder="Portfolio name" className="min-w-52 flex-1 px-3 py-2" /><select name="baseCurrency" className="px-3 py-2"><option>INR</option><option>USD</option></select><button className="btn-primary px-5 py-2" disabled={create.isPending}>{create.isPending ? 'Creating…' : 'Create portfolio'}</button></form>}
