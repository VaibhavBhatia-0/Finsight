...
export default function PortfolioDetailPage() {
  ...
  return <main>
    ...
    <header className="page-heading"><div><p className="page-eyebrow">Portfolio · {value.portfolio.baseCurrency}</p><h1>{value.portfolio.name}</h1><p className="page-subtitle">Position values and P&amp;L are derived from the chronological transaction ledger.</p></div><button type="button" onClick={() => setShowForm(current => !current)} className="btn-primary inline-flex items-center gap-2 px-4 py-2"><Plus size={16} /> Add transaction</button></header>
    ...
    <div className="content-stack">
      ...
      {value.risk.status && <div className="notice p-3 text-sm">Risk metrics unavailable: {value.risk.status.replace(/_/g, ' ').toLowerCase()}.</div>}
