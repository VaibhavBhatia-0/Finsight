...
const ExpensesPage: React.FC = () => {
  ...
  return (
    <main>
      <header className="page-heading"><div><p className="page-eyebrow">Personal finance</p><h1>Expenses</h1><p className="page-subtitle">Track categorized outflows without losing their original transaction currency.</p></div><button onClick={() => setShowForm(current => !current)} className="btn-primary px-4 py-2">Add expense</button></header>
      <FinanceNav />
        ...
        <ul className="panel data-list">
          {expenses.map((e: Expense) => (
            <li key={e.id} className="data-row">
              <span><strong>{e.category}</strong><span className="ml-3 font-mono">{e.currency} {e.amount.toLocaleString()}</span><span className="ml-3 text-sm text-gray-500">{e.date}</span></span><button disabled={remove.isPending} onClick={() => remove.mutate(e.id)} className="text-sm text-red-600">Delete</button>
            </li>
          ))}
        </ul>
      ) : (
